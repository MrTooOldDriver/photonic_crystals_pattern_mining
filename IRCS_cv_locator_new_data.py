...
def find_circle_threshold_ver(image_path, TARGET_IMAGE_SIZE):
    ...
    no_circle_detection_original_canny_img = cv2.Canny(no_circle_detection_original_img, 20, 25)
...
if __name__ == '__main__':
    ...
    pathlib.Path('./output_new_data').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/rgb').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/rgb_no_circle_det').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/canny').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/canny_no_circle_det').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg_fill').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg_fix_circle_mask').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg_fix_circle_mask_canny').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/rgb_zoom').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/hist').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg_hist').mkdir(parents=True, exist_ok=True)
    pathlib.Path('./output_new_data/no_bg_bin_hist').mkdir(parents=True, exist_ok=True)

    for path in data_dir.glob('*/*/*.jpg'):
        print('processing ' + str(path))
        # skip black images
        if path.parent.parent.name == 'C4_Black':
            continue
        img = cv2.imread(str(path))
        image_name = path.parent.parent.name + '-' + path.parent.name + '-' +str(i) + '.jpg'
        ...
        cv2.imwrite('./output_new_data/rgb/{0}'.format(image_name), resized_img)
        cv2.imwrite('./output_new_data/rgb_no_circle_det/{0}'.format(image_name), no_circle_detection_original_img)
        cv2.imwrite('./output_new_data/canny/{0}'.format(image_name), bounding_rect)
        cv2.imwrite('./output_new_data/canny_no_circle_det/{0}'.format(image_name), no_circle_detection_original_canny_img)
        cv2.imwrite('./output_new_data/no_bg/{0}'.format(image_name), no_background_img)
        cv2.imwrite('./output_new_data/no_bg_fill/{0}'.format(image_name), img_with_bg_fill)
        cv2.imwrite('./output_new_data/no_bg_fix_circle_mask/{0}'.format(image_name), no_bg_fix_circle_mask)
        cv2.imwrite('./output_new_data/no_bg_fix_circle_mask_canny/{0}'.format(image_name), no_bg_fix_circle_mask_canny)
        cv2.imwrite('./output_new_data/rgb_zoom/{0}'.format(image_name),resized_cropped_img_future)
        ax.figure.savefig('./output_new_data/hist/{0}'.format(image_name))
        no_bg_ax.figure.savefig('./output_new_data/no_bg_hist/{0}'.format(image_name))
        no_bg_bin_ax.figure.savefig('./output_new_data/no_bg_bin_hist/{0}'.format(image_name))
        i += 1
